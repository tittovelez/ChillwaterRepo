    ...
    public void setSlot(GameObject slot, int pos, int cant)
    {
        ...
        for (int i = 0; i < slots.Length; i++)
        {
            if (slots[i] != null)
            {
                ...
                {
                    int already_cant = slots[i].GetComponent<AttributeController>().getCantidad();
                    this.slots[i].GetComponent<AttributeController>().setCantidad(aread)
                }
            }
        }
    }
                ...
                { 
                    ...
                    for (int e = 0; e < inventario.Length; e++)
                    {
                        ...
                        if (child.tag == "slot" && child.transform.childCount <= 1 && !slotUsed)
                        {
                            ...
                            item.transform.localPosition = new Vector3(e, 0, 0);
